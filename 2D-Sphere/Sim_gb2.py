...
def curvatp_gb_ensemble(p,indnbr,Da,Oplist,dnbr):
    ...
    for time in range(K):
        ind = np.random.choice(nbr,2,replace = False)
        ind = np.append(p,ind)
        sample_res[time] = curvatp_gb_simp(p,ind,Oplist,dnbr)
    if(np.sum(~np.isnan(sample_res))==0):
        return(np.nan)
    else:
        res = np.median(sample_res[~np.isnan(sample_res)])
    return(res)
...
#       res_quadfit[p] = curvatp_quadfit(p,indnbr,Da,dmat,wmat,d, Oplist[p])
    ...
    if(np.sum(~np.isnan(res_gb))==0):
        ...
        continue
    print("median: ",np.median(res_gb[~np.isnan(res_gb)]))
    print("mean: ",np.mean(res_gb[~np.isnan(res_gb)]))
    print("variance: ",np.var(res_gb[~np.isnan(res_gb)]))
    print(res_gb)
t2 = time.time()
print("time: ",t2-t1)
